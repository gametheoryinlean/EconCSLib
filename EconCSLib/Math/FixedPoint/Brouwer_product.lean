...
Brouwer's fixed-point theorem for products of standard simplices, proved by retraction
onto a single standard simplex.

## Setup

Let `I` be a finite linearly ordered type and `card : I → ℕ+`.  Define:

* `ProductSimplices card` — the product `Πᵢ stdSimplex ℝ (Fin (card i))` of standard simplices.
* `BigSimplex card` — the single standard simplex `stdSimplex ℝ (Fin (total_card card))`
  where `total_card card = ∑ i, card i`.

The idea is that `BigSimplex` naturally embeds `ProductSimplices` as a retract.

## Key constructions

* `embed_from_product` — embeds a product point into the big simplex by concatenating the
  blocks; the `i`-th block occupies coordinates `[prefix_sum i, prefix_sum i + card i)`.
* `project_to_product` — projects a big-simplex point back to the product; the `i`-th
  component is the renormalised restriction to block `i` (using `tPush` to handle the
  degenerate case where a block sum is zero).
* `project_embed_id` — `project_to_product ∘ embed_from_product = id`.

## Main result

* `Brouwer_Product` — every continuous self-map of `ProductSimplices card` has a fixed point.
