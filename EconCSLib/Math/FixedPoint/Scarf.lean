...
* `T` — a finite inhabited type whose elements play the role of **vertices** (or lattice points).
* `I` — a finite type whose elements play the role of **coordinate directions** (or colors).
...
* `IndexedLOrder I T` — typeclass: a family `I → LinearOrder T` of linear orders on `T`.
* `isDominant σ C` — `σ` dominates every point of `T` using some order from `C`.
* `isRoom σ C` — dominant cell with `|σ| = |C|`  (a "full-rank" cell).
* `isDoor σ C` — dominant cell with `|C| = |σ| + 1`  (one surplus color).
* `isColorful c σ C` — room where the coloring `c : T → I` maps `σ` **bijectively** onto `C`.
* `colorful c` — the `Finset` of all colorful rooms `(σ, C)`.

## Main result

* `Scarf` — for any coloring `c : T → I` (with `[Inhabited I]`), `IST.colorful c` is nonempty:
  there exists a colorful room `(σ, C)`.

## Proof strategy

The proof uses a **parity argument** on a bipartite counting set:

1. Define a double-counting set `dbcountingset c i` pairing each nearly-colorful room with its
   adjacent colorful and outside-door rooms.
2. Show that the number of *typed nearly-colorful rooms* of each type is **odd**
   (`typed_colorful_room_odd`), using an induction that tracks inside/outside doors.
3. Since an odd number is positive, the set of colorful rooms is nonempty.
...
* Shapley, L. S., "On Balanced Games Without Side Payments", in *Mathematical Programming*
  (1973), pp. 261–290.
-/
...
variable {X : Type u} {Y : Type v} [DecidableEq X] [DecidableEq Y]

lemma injOn_sdiff (s : Finset X) (f : X → Y) (h : s.card = (Finset.image f s).card + 1) : ∃ a b, a ∈ s ∧ b ∈ s ∧ f a = f b ∧ a ≠ b ∧ Set.InjOn f (s \ ({a, b} : Finset X)) := by
  have of_card_domain_eq_card_image_succ  (s : Finset X) (f : X → Y) (h : s.card = (Finset.image f s).card + 1) :
  ...
  have : (Finset.image f (s \ {a, b})).card = (Finset.image f s).card - 1 := by
    have aux1 : ∀ c, c ∈ s → c ≠ a → c ≠ b → f c ≠ f a := by
      ...
      have cardabc : ({a, b, c} : Finset X).card = 3 := by
      ...
      have : (image f s).card < s.card - 1 :=
        calc
          ...
          _ = s.card - 3 + 1 := by
            rw [Finset.card_sdiff, Finset.inter_eq_left.mpr abcss, cardabc]
          _ < _ := by
            have : 2 < s.card := by
    ...
    rw [aux2, Finset.card_sdiff, Finset.inter_comm,
        Finset.inter_singleton_of_mem (by simp; exact ⟨a, as, rfl⟩), card_singleton]
...
/-- `σ` is **dominant** with respect to `C`: for every `y : T` there exists `i ∈ C`
    ...
    This is the key structural invariant of Scarf's lemma.  It generalises the notion
    that a simplex vertex set "covers" a given point in its convex hull. -/
def isDominant  :=
  ∀ y, ∃ i ∈ C, ∀ x ∈ σ,  y ≤[i] x
...
/-- `(σ, C)` is a **door**: a dominant cell with `|C| = |σ| + 1`.
    ...
    A door has one more color than points; it is adjacent to exactly two rooms in the
    Scarf complex.  Doors are the "walls" between rooms in the parity argument. -/
...
theorem _root_.Finset.card_filter_filter_neg {X : Type*} (s : Finset X) (p : X → Prop) [DecidablePred p]
 : s.card  = (Finset.filter p s).card + (Finset.filter (fun (a : X) => ¬p a) s).card :=
  by
