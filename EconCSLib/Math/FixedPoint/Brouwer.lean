...
Brouwer's fixed-point theorem for self-maps of the standard simplex, proved via
Scarf's combinatorial lemma.

## Main result

* `Brouwer` — every continuous map `f : stdSimplex ℝ (Fin n) → stdSimplex ℝ (Fin n)`
  has a fixed point.
...
1. **Coloring** (`Fcolor`): For each `x : TT n l`, pick a coordinate `j` where
   `(f x)_j ≥ x_j` — i.e., `f` moves `x` in the `j`-th direction.  This is
   well-defined since `∑ x j = 1 = ∑ (f x) j` forces at least one coordinate to
   increase.

2. **Scarf's lemma** (`TT.ILO.Scarf`): Applied with the coloring `Fcolor`, yields a
   colorful room `(σ, C)` at level `l` — a set of `n` lattice points covering all `n`
   directions.

3. **Limit** (`room_seq`, `hpkg`): As `l → ∞`, extract a subsequence of colorful rooms
   via `exists_subseq_constant_of_finite_image` (pigeonhole on the finite color set `C`),
   then use compactness of the simplex to obtain a convergent subsequence of the
   representative room points.

4. **Fixed point**: The limit point `z` satisfies `f(z) ≥ z` coordinatewise (by
   passing the coloring property through the limit).  Since `∑ z_j = 1 = ∑ f(z)_j`,
   coordinatewise inequality forces `f(z) = z`.
...
* Todd, M. J., "The Computation of Fixed Points and Applications",
...
(h2 : σ.Nonempty):
  l < ∑ k ∈ C, (σ.image (fun x => (x k : ℕ))).min' (h2.image _) + C.card := by
  by_contra h_not
  push Not at h_not
...
def hpkg_aux:
  ...
  obtain ⟨z, hz, subseq, subseq_mono, h_tendsto⟩ := h_compact.tendsto_subseq h_in_univ
  use ⟨z, subseq⟩
  exact ⟨subseq_mono, h_tendsto⟩

def hpkg := Classical.choice  (hpkg_aux f)

/-
Use
-- StrictMono.id_le : ∀ {subseq : ℕ → ℕ}, StrictMono subseq → ∀ (n : ℕ), n ≤ subseq n
-- OrderEmbedding.strictMono: (f : X ↪o Y), StrictMono ⇑f
-/
...
theorem tendsto_diam_to_zero (f : stdSimplex ℝ (Fin n) → stdSimplex ℝ (Fin n)) :
  Tendsto
    (fun k =>
      Metric.diam
        ((((room_seq f (g1 f ((hpkg f).1.2 k))).1.1.image (fun x => TTtostdSimplex x)) :
...
theorem f_coords_ge_z_coords (f : stdSimplex ℝ (Fin n) → stdSimplex ℝ (Fin n)) (hf : Continuous f) :
  ∀ i ∈ (gpkg f).1.1, (f (hpkg f).1.1).1 i ≥ ((hpkg f).1.1).1 i := by
      let z := (hpkg f).1.1
      let C := (gpkg f).1.1
      let subseq := (hpkg f).1.2
      have convergence_to_z : Filter.Tendsto ((fun l' => (room_point_seq f (g1 f l'): stdSimplex ℝ (Fin n))) ∘ subseq) Filter.atTop (𝓝 z) := by
        exact (hpkg f).2.2
      ...
      have y_seq_subseq_converges_to_z : Filter.Tendsto (y_seq ∘ subseq) Filter.atTop (𝓝 z) := by
        have h_dist_tends_to_zero : Filter.Tendsto (fun k => dist (y_seq (subseq k)) ((fun l' => (room_point_seq f (g1 f l') : stdSimplex ℝ (Fin n))) (subseq k))) Filter.atTop (𝓝 0) := by
          have h_bound : ∀ k, dist (y_seq (subseq k)) ((room_point_seq f (g1 f (subseq k)) : stdSimplex ℝ (Fin n))) ≤
                Metric.diam ((((room_seq f (g1 f (subseq k))).1.1.image (fun x => TTtostdSimplex x)) : Set (stdSimplex ℝ (Fin n)))) := by
            intro k
            apply Metric.dist_le_diam_of_mem
            · exact Set.Finite.isBounded (Finset.finite_toSet _)
            · exact Finset.mem_image_of_mem TTtostdSimplex (y_seq_spec (subseq k)).1
            · exact Finset.mem_image_of_mem TTtostdSimplex (pick_colorful_point ((Finset.mem_filter.1 (room_seq f (g1 f (subseq k))).2).2)).2
          have h_diam_tendsto : Tendsto (fun k => Metric.diam ((((room_seq f (g1 f (subseq k))).1.1.image TTtostdSimplex) : Set (stdSimplex ℝ (Fin n))))) atTop (𝓝 0) := by
        ...
        intro k ⟨hk1, hk2⟩
        calc dist (y_seq (subseq k)) z
          ≤ dist (y_seq (subseq k)) ((room_point_seq f (g1 f (subseq k)) : stdSimplex ℝ (Fin n)))
            + dist ((room_point_seq f (g1 f (subseq k)) : stdSimplex ℝ (Fin n))) z := dist_triangle _ _ _
        _ < ε / 2 + ε / 2 := add_lt_add (by simp at hk2; exact hk2) hk1
        _ = ε := add_halves ε

      have f_y_seq_subseq_converges_to_f_z : Filter.Tendsto (f ∘ y_seq ∘ subseq) Filter.atTop (𝓝 (f z)) := by
        exact hf.continuousAt.tendsto.comp y_seq_subseq_converges_to_z
      have f_y_seq_subseq_coord_converges : Filter.Tendsto (fun l' => (f (y_seq (subseq l'))).1 idx) Filter.atTop (𝓝 ((f z).1 idx)) := by
        have h_continuous : Continuous (fun x : stdSimplex ℝ (Fin n) => x.1 idx) :=
          Continuous.comp (continuous_apply idx) continuous_subtype_val
        exact h_continuous.continuousAt.tendsto.comp f_y_seq_subseq_converges_to_f_z
      have y_seq_subseq_coord_converges : Filter.Tendsto (fun l' => (y_seq (subseq l')).1 idx) Filter.atTop (𝓝 (z.1 idx)) := by
        have h_continuous : Continuous (fun x : stdSimplex ℝ (Fin n) => x.1 idx) :=
          Continuous.comp (continuous_apply idx) continuous_subtype_val
        exact h_continuous.continuousAt.tendsto.comp y_seq_subseq_converges_to_z

      exact le_of_tendsto_of_tendsto y_seq_subseq_coord_converges f_y_seq_subseq_coord_converges (Eventually.of_forall (fun l' => h_ineq (subseq l')))
...
theorem Brouwer (hf : Continuous f): ∃ x , f x = x := by
  ...
  have convergence_to_z : Filter.Tendsto ((fun l' => (room_point_seq f (g1 f l'): stdSimplex ℝ (Fin n))) ∘ subseq) Filter.atTop (𝓝 z) :=
    (hpkg f).2.2
  ...
  have coords_outside_C_zero : ∀ i_1 ∉ C, z.1 i_1 = 0 := by
    ...
    have h_tendsto_coord_z : Tendsto (fun k => ((room_point_seq f (g1 f (subseq k))) : stdSimplex ℝ (Fin n)).1 i_1) atTop (𝓝 (z.1 i_1)) := by
    ...
    have tendsto_zero_subseq : Tendsto (fun k => ((room_point_seq f (g1 f (subseq k))) : stdSimplex ℝ (Fin n)).1 i_1) atTop (𝓝 0) :=
