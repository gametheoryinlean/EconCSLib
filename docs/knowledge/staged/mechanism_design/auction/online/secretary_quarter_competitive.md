...
`competitive` assumes $v$ is **injective**. The obstruction to dropping
this is value-ties *at the threshold*: in the welfare step injectivity is
exactly what makes the pre-argmax bids *strictly* below the threshold, so
the weak acceptance rule $p \le b$ does not let a tied bidder clear the
price early; in the counting step it pins the argmax and second-largest to
unique positions.

The two-type-parameter design `SingleItemAuction B F` — where the
auction's price rule sees `(identity, value)` pairs — provides the hook for
a future tie-breaking extension: a compatible total order on the identity
type `B` could break value ties, recovering a constant guarantee for all
